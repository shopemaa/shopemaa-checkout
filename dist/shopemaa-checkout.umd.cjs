(function(b,m){typeof exports=="object"&&typeof module<"u"?m(exports):typeof define=="function"&&define.amd?define(["exports"],m):(b=typeof globalThis<"u"?globalThis:b||self,m(b["Shopemaa checkout"]={}))})(this,function(b){"use strict";function m(e,a){E(e),q(a),V(),x(),D(),ee(),w()}function E(e){localStorage.setItem("shopemaa_store_key",e)}function j(){return localStorage.getItem("shopemaa_store_key")}function q(e){localStorage.setItem("shopemaa_store_secret",e)}function A(){return localStorage.getItem("shopemaa_store_secret")}function x(){localStorage.getItem("shopemaaCheckoutBucket")===null&&g()}function g(){let e={cartId:null,items:[],shippingCharge:0,paymentFee:0,discount:0,paymentMethodId:null,shippingMethodId:null,initAt:new Date().getMilliseconds()};localStorage.setItem("shopemaaCheckoutBucket",JSON.stringify(e))}function c(){let e=localStorage.getItem("shopemaaCheckoutBucket");return e===null&&(x(),e=localStorage.getItem("shopemaaCheckoutBucket")),JSON.parse(e)}function v(e){e.items=e.items.sort((a,t)=>a.orderIndex-t.orderIndex),localStorage.setItem("shopemaaCheckoutBucket",JSON.stringify(e))}function D(){N(),P(),O()}function N(){$(".shopemaa-buy-btn").click(function(){let e=$(this).data("productId");F(e)}),$(".shopemaa-stock-up").click(function(){let e=$(this).data("productId");S(e,1)}),$(".shopemaa-stock-down").click(function(){let e=$(this).data("productId");S(e,-1)})}function w(){let e=c(),a=document.getElementsByClassName("shopemaa-cart-items-count");if(a!=null){let t=a.length,o=0;e.items.forEach(n=>{o+=n.qty});for(let n=0;n<t;n++)a.item(n).innerText=o}G()}function G(){let e=document.getElementsByClassName("shopemaa-cart-item-qty");if(e!=null){let a=e.length;for(let t=0;t<a;t++){let o=e.item(t),n=c(),i=o.dataset.productId,l=n.items.find(f=>f.id===i);l!=null?e.item(t).innerText=l.qty:e.item(t).innerText=0}}}function P(){$(".shopemaa-cart-btn").click(function(){y(!0)})}function O(){$(".shopemaa-order-track-btn").click(function(){ne()})}function F(e){C(e).then(a=>{a.ok&&a.json().then(t=>{let o=t.data.product;o.stock>0&&k(o,1)}).catch(t=>{d(t)})}).catch(a=>{d(a)})}function S(e,a){C(e).then(t=>{t.ok&&t.json().then(o=>{let n=o.data.product;n.stock>=a&&k(n,a)}).catch(o=>{d(o)})}).catch(t=>{d(t)})}function I(e,a){c().items.forEach(o=>{let n=`shopemaa_cart_item_${o.id}`,i=document.getElementById(n);i!=null&&i.remove()}),k(e,a)}function k(e,a){let t=c(),o=t.items.find(n=>n.id===e.id);o==null?t.items.push({id:e.id,name:e.name,stock:e.stock,qty:1,fullImages:e.fullImages,productSpecificDiscount:e.productSpecificDiscount,price:e.price,orderIndex:t.items.length+1}):o.qty+a<=e.stock&&(o.qty+=a,t.items=t.items.filter(n=>n.id!==e.id),o.qty>0&&t.items.push(o)),v(t),y(!0),w()}function C(e){let a=`query { product(productId: "${e}") { id name price stock fullImages productSpecificDiscount attributes { id name values isRequired } } }`;return p(a)}function p(e){return fetch(L(),{method:"POST",headers:{"Content-Type":"application/json","store-key":j(),"store-secret":A()},body:JSON.stringify({query:e})})}function L(){return"https://api.shopemaa.com/query"}function V(){p("query { storeBySecret { name title description currency isOpen } }").then(a=>{a.ok&&a.json().then(t=>{localStorage.setItem("shopemaaStoreInfo",JSON.stringify(t.data.storeBySecret))}).catch(t=>{d(t)})}).catch(a=>{d(a)})}function R(){return JSON.parse(localStorage.getItem("shopemaaStoreInfo"))}function s(){return R().currency}function h(e){let a=e.price;return e.productSpecificDiscount!==0&&(a=a-e.productSpecificDiscount*a/100),a}function H(){let e=0;c().items.forEach(a=>{e+=a.qty*h(a)}),document.getElementById("shopemaa_cart-subtotal").innerText=(e/100).toFixed(2)+" "+s()}function U(){let e=c(),a=0;e.items.forEach(o=>{a+=o.qty*h(o)});let t=a;document.getElementById("shopemaa_cart-grand-total").innerText=(t/100).toFixed(2)+" "+s()}function y(e){if(e){let a=document.getElementById("shopemaa_cartModal");a==null&&K();let t=document.getElementById("shopemaa_cartItemsView");c().items.forEach(o=>{t.appendChild(z(o));let n=document.getElementById(`shopemaa_qty_minus_${o.id}`);n.onclick=function(){I(o,-1)};let i=document.getElementById(`shopemaa_qty_plus_${o.id}`);i.onclick=function(){I(o,1)}}),H(),U()}else{let a=document.getElementById("shopemaa_cartModal");a!=null&&a.remove()}}function z(e){let a=e.fullImages[0],t=h(e),o=document.createElement("button");o.classList.add("flex","w-3.5","h-3.5","px-px","items-center","justify-center","bg-black","hover:bg-indigo-500","rounded","transition","duration-100"),o.innerHTML='<div class="h-px mx-px w-full bg-white"></div>',o.id=`shopemaa_qty_minus_${e.id}`;let n=document.createElement("button");n.classList.add("flex","w-3.5","h-3.5","px-px","items-center","justify-center","bg-black","hover:bg-indigo-500","rounded","transition","duration-100"),n.innerHTML='<div class="relative h-full w-full py-px"> <div class="absolute top-1/2 left-0 h-px w-full bg-white"></div> <div class="inline-block max-w-max mx-auto h-full bg-white"> <div class="inline-block px-px"></div> </div> </div>',n.id=`shopemaa_qty_plus_${e.id}`;let i=`<div class="flex mb-6 justify-between items-center">
                  ...
                  </div>`,l=document.createElement("div");return l.id=`shopemaa_cart_item_${e.id}`,l.innerHTML=i,l}function K(){let e=`<div class="fixed overflow-y-auto z-50 top-0 left-0 w-full h-full bg-gray-900 bg-opacity-80 pb-3">
      ...
      </div>`,a=document.createElement("section");a.classList.add("relative"),a.id="shopemaa_cartModal",a.innerHTML+=e,document.body.appendChild(a)}function J(){let e=c();e.items.length!==0&&(u(!1),e.shippingCharge=0,e.paymentFee=0,e.discount=0,e.paymentMethodId=null,e.shippingMethodId=null,v(e),W())}function W(){let e=c(),a="[";if(e.items.forEach(t=>{a+=`{ productId: "${t.id}" quantity: ${t.qty} }`}),a+="]",e.cartId===null){let t="mutation { newCart(params: { cartItems: "+a+" }) { id isShippingRequired } }";B(p(t),!0)}else{let t=`mutation { updateCart(id: "${e.cartId}" params: { cartItems: `+a+" }) { id isShippingRequired } }";B(p(t),!1)}}function B(e,a){e.then(t=>{if(!t.ok){d(`create or update cart response is: ${t.statusText}`),u(!0);return}t.json().then(o=>{if(o.data===null){d("create or update cart response body is NULL"),u(!0);return}if(a){let n=o.data.newCart.id,i=c();i.cartId=n,v(i)}y(!1),T()}).catch(o=>{u(!0),d(o)})}).catch(t=>{u(!0),d(t)})}function u(e){if(e){let a=document.getElementById("shopemaa_checkoutBtn");a.style.display="block";let t=document.getElementById("shopemaa_creatingCartBtn");t.style.display="none"}else{let a=document.getElementById("shopemaa_checkoutBtn");a.style.display="none";let t=document.getElementById("shopemaa_creatingCartBtn");t.style.display="block"}}function M(){let e=c(),a=0;e.items.forEach(l=>{a+=l.qty*h(l)}),document.getElementById("shopemaa_checkout-subtotal").innerText=(a/100).toFixed(2)+" "+s();let t=e.discount;document.getElementById("shopemaa_checkout-discount").innerText="-"+(t/100).toFixed(2)+" "+s();let o=e.shippingCharge;document.getElementById("shopemaa_checkout-shipping-charge").innerText=(o/100).toFixed(2)+" "+s();let n=e.paymentFee;document.getElementById("shopemaa_checkout-payment-fee").innerText=(n/100).toFixed(2)+" "+s();let i=((a+o+n-t)/100).toFixed(2);document.getElementById("shopemaa_checkout-grand-total").innerText=i+" "+s()}function Q(e){if(e)T();else{let a=document.getElementById("shopemaa_checkoutModal");a!=null&&a.remove()}}function Y(e){if(document.getElementById("shopemaa_coupon").value.trim()==="")return;let a=document.getElementById("shopemaa_coupon").value,t=c(),o="",n=document.getElementById("shopemaa_shippingMethod").value;n!=null&&n!=="select"&&(o=`shippingMethodId: "${n}"`);let i=`query { checkDiscountForGuests(couponCode: "${a}" cartId: "${t.cartId}" ${o}) }`;p(i).then(l=>{l.ok&&l.json().then(f=>{if(f.data===null)return;let ie=f.data.checkDiscountForGuests;t=c(),t.discount=ie,v(t),M()}).catch(f=>{d(f)})}).catch(l=>{d(l)})}function T(){let e=`<div class="fixed overflow-y-auto z-50 top-0 left-0 w-full h-full bg-gray-900 bg-opacity-80">
    <form>
       <div class="relative ml-auto w-full max-w-lg bg-white">
          <div class="p-6 border-b-2 border-black">
             <h3 class="text-2xl font-bold">Your Cart</h3>
 ...
 </div>`,a=document.createElement("section");a.classList.add("relative"),a.id="shopemaa_checkoutModal",a.innerHTML+=e,document.body.appendChild(a),M(),Z(),X()}function Z(){p("query { shippingMethods { id displayName deliveryCharge deliveryTimeInDays WeightUnit isFlat isActive } }").then(a=>{a.ok&&a.json().then(t=>{if(t.data!==null){let o=t.data.shippingMethods,n=document.getElementById("shopemaa_shippingMethod");o.forEach(i=>{let l=document.createElement("option");l.id=i.id,l.value=i.id,i.deliveryTimeInDays===1||i.deliveryTimeInDays===0?l.innerText=i.displayName+" (Delivering today)":l.innerText=i.displayName+` (Approx. delivery in ${i.deliveryTimeInDays})`,n.appendChild(l)})}}).catch(t=>{d(t)})}).catch(a=>{d(a)})}function X(){p("query { paymentMethods { id displayName currencyName currencySymbol isDigitalPayment } }").then(a=>{a.ok&&a.json().then(t=>{if(t.data!==null){let o=t.data.paymentMethods,n=document.getElementById("shopemaa_paymentMethod");o.forEach(i=>{let l=document.createElement("option");l.id=i.id,l.value=i.id,l.innerText=i.displayName,n.appendChild(l)})}}).catch(t=>{d(t)})}).catch(a=>{d(a)})}function r(){return"gray"}function d(e){console.log(e)}function ee(){let e=864e5,a=function(){console.log("Performing cart cleanup");let t=c(),o=1e3*60*60*24,n=t.initAt;n==null&&g(),new Date().getMilliseconds()-n>=o&&g()};setInterval(a,e),a()}function ae(){let e=document.getElementById("shopemaa_orderModal");e!==null&&e.remove()}function te(e){let a=`<div class="fixed overflow-y-auto z-50 top-0 left-0 w-full h-full bg-gray-900 bg-opacity-80 pb-3">
      ...
      </div>`,t=document.createElement("section");t.classList.add("relative"),t.id="shopemaa_orderModal",t.innerHTML+=a,document.body.appendChild(t)}function _(){let e=document.getElementById("shopemaa_orderSearchModal");e!==null&&e.remove()}function oe(){let e=document.getElementById("shopemaa_order-hash").value,a=document.getElementById("shopemaa_customer-email").value,t=`query { orderByCustomerEmail(hash: "${e}", email: "${a}") { id hash shippingCharge paymentProcessingFee subtotal grandTotal discountedAmount status paymentStatus createdAt updatedAt billingAddress { id street streetTwo city state postcode email phone location { id name shortCode } } shippingAddress { id street streetTwo city state postcode email phone location { id name shortCode } } cart { isShippingRequired cartItems { product { id name slug description fullImages isDigitalProduct productUnit } quantity purchasePrice attributes { name selectedValue } variation { id name price sku stock } } } customer { email phone firstName lastName profilePicture } paymentMethod { id displayName currencyName currencySymbol isDigitalPayment } shippingMethod { id displayName deliveryCharge deliveryTimeInDays WeightUnit isFlat isActive } couponCode { code } payments { isPaid payableAmount gatewayName } } }`;p(t).then(o=>{o.ok&&o.json().then(n=>{n.data!==null&&(_(),te(n.data.orderByCustomerEmail))}).catch(n=>{d(n)})}).catch(o=>{d(o)})}function ne(){let e=`<div class="fixed overflow-y-auto z-50 top-0 left-0 w-full h-full bg-gray-900 bg-opacity-80 pb-3">
          <div class="relative ml-auto w-full h-full max-w-lg bg-white">
              <div class="p-6 border-b-2 border-black">
      ...
      </div>`,a=document.createElement("section");a.classList.add("relative"),a.id="shopemaa_orderSearchModal",a.innerHTML+=e,document.body.appendChild(a)}(async function(){typeof window<"u"&&(window.shopemaa=window.shopemaa||{},window.shopemaa.hideOrderSearchModal=_,window.shopemaa.getOrderDetails=oe,window.shopemaa.onGotoCheckout=J,window.shopemaa.toggleCartView=y,window.shopemaa.applyDiscount=Y,window.shopemaa.hideOrderDetailsModal=ae,window.shopemaa.toggleCheckoutView=Q,window.shopemaa.toggleCheckoutBtn=u)})(),b.initShopemaa=m,Object.defineProperty(b,Symbol.toStringTag,{value:"Module"})});
