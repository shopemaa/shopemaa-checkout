(function(p,u){typeof exports=="object"&&typeof module<"u"?u(exports):typeof define=="function"&&define.amd?define(["exports"],u):(p=typeof globalThis<"u"?globalThis:p||self,u(p["Shopemaa checkout"]={}))})(this,function(p){"use strict";function u(e,a){M(e),_(a),O(),x(),j(),Q(),k()}function M(e){localStorage.setItem("shopemaa_store_key",e)}function T(){return localStorage.getItem("shopemaa_store_key")}function _(e){localStorage.setItem("shopemaa_store_secret",e)}function E(){return localStorage.getItem("shopemaa_store_secret")}function x(){localStorage.getItem("shopemaaCheckoutBucket")===null&&h()}function h(){let e={cartId:null,items:[],shippingCharge:0,paymentFee:0,discount:0,paymentMethodId:null,shippingMethodId:null,initAt:new Date().getMilliseconds()};localStorage.setItem("shopemaaCheckoutBucket",JSON.stringify(e))}function s(){let e=localStorage.getItem("shopemaaCheckoutBucket");return e===null&&(x(),e=localStorage.getItem("shopemaaCheckoutBucket")),JSON.parse(e)}function y(e){e.items=e.items.sort((a,t)=>a.orderIndex-t.orderIndex),localStorage.setItem("shopemaaCheckoutBucket",JSON.stringify(e))}function j(){q(),N(),P()}function q(){$(".shopemaa-buy-btn").click(function(){let e=$(this).data("productId");G(e)}),$(".shopemaa-stock-up").click(function(){let e=$(this).data("productId");w(e,1)}),$(".shopemaa-stock-down").click(function(){let e=$(this).data("productId");w(e,-1)})}function k(){let e=s(),a=document.getElementsByClassName("shopemaa-cart-items-count");if(a!=null){let t=a.length,o=0;e.items.forEach(n=>{o+=n.qty});for(let n=0;n<t;n++)a.item(n).innerText=o}A()}function A(){let e=document.getElementsByClassName("shopemaa-cart-item-qty");if(e!=null){let a=e.length;for(let t=0;t<a;t++){let o=e.item(t),n=s(),i=o.dataset.productId,l=n.items.find(ae=>ae.id===i);l!=null?e.item(t).innerText=l.qty:e.item(t).innerText=0}}}function N(){$(".shopemaa-cart-btn").click(function(){v(!0)})}function P(){$(".shopemaa-order-track-btn").click(function(){ee()})}function G(e){I(e).then(a=>{a.ok&&a.json().then(t=>{let o=t.data.product;o.stock>0&&g(o,1)}).catch(t=>{d(t)})}).catch(a=>{d(a)})}function w(e,a){I(e).then(t=>{t.ok&&t.json().then(o=>{let n=o.data.product;n.stock>=a&&g(n,a)}).catch(o=>{d(o)})}).catch(t=>{d(t)})}function S(e,a){s().items.forEach(o=>{let n=`shopemaa_cart_item_${o.id}`,i=document.getElementById(n);i!=null&&i.remove()}),g(e,a)}function g(e,a){let t=s(),o=t.items.find(n=>n.id===e.id);o==null?t.items.push({id:e.id,name:e.name,stock:e.stock,qty:1,fullImages:e.fullImages,productSpecificDiscount:e.productSpecificDiscount,price:e.price,orderIndex:t.items.length+1}):o.qty+a<=e.stock&&(o.qty+=a,t.items=t.items.filter(n=>n.id!==e.id),o.qty>0&&t.items.push(o)),y(t),v(!0),k()}function I(e){let a=`query { product(productId: "${e}") { id name price stock fullImages productSpecificDiscount attributes { id name values isRequired } } }`;return b(a)}function b(e){return fetch(D(),{method:"POST",headers:{"Content-Type":"application/json","store-key":T(),"store-secret":E()},body:JSON.stringify({query:e})})}function D(){return"https://api.shopemaa.com/query"}function O(){b("query { storeBySecret { name title description currency isOpen } }").then(a=>{a.ok&&a.json().then(t=>{localStorage.setItem("shopemaaStoreInfo",JSON.stringify(t.data.storeBySecret))}).catch(t=>{d(t)})}).catch(a=>{d(a)})}function F(){return JSON.parse(localStorage.getItem("shopemaaStoreInfo"))}function c(){return F().currency}function m(e){let a=e.price;return e.productSpecificDiscount!==0&&(a=a-e.productSpecificDiscount*a/100),a}function L(){let e=0;s().items.forEach(a=>{e+=a.qty*m(a)}),document.getElementById("shopemaa_cart-subtotal").innerText=(e/100).toFixed(2)+" "+c()}function V(){let e=s(),a=0;e.items.forEach(o=>{a+=o.qty*m(o)});let t=a;document.getElementById("shopemaa_cart-grand-total").innerText=(t/100).toFixed(2)+" "+c()}function v(e){if(e){let a=document.getElementById("shopemaa_cartModal");a==null&&H();let t=document.getElementById("shopemaa_cartItemsView");s().items.forEach(o=>{t.appendChild(R(o));let n=document.getElementById(`shopemaa_qty_minus_${o.id}`);n.onclick=function(){S(o,-1)};let i=document.getElementById(`shopemaa_qty_plus_${o.id}`);i.onclick=function(){S(o,1)}}),L(),V()}else{let a=document.getElementById("shopemaa_cartModal");a!=null&&a.remove()}}function R(e){let a=e.fullImages[0],t=m(e),o=document.createElement("button");o.classList.add("flex","w-3.5","h-3.5","px-px","items-center","justify-center","bg-black","hover:bg-indigo-500","rounded","transition","duration-100"),o.innerHTML='<div class="h-px mx-px w-full bg-white"></div>',o.id=`shopemaa_qty_minus_${e.id}`;let n=document.createElement("button");n.classList.add("flex","w-3.5","h-3.5","px-px","items-center","justify-center","bg-black","hover:bg-indigo-500","rounded","transition","duration-100"),n.innerHTML='<div class="relative h-full w-full py-px"> <div class="absolute top-1/2 left-0 h-px w-full bg-white"></div> <div class="inline-block max-w-max mx-auto h-full bg-white"> <div class="inline-block px-px"></div> </div> </div>',n.id=`shopemaa_qty_plus_${e.id}`;let i=`<div class="flex mb-6 justify-between items-center">
                      ...
                      <div class="w-auto">
                          <span class="text-xl font-bold">${(t/100).toFixed(2)} ${c()}</span>
                      </div>
                  </div>`,l=document.createElement("div");return l.id=`shopemaa_cart_item_${e.id}`,l.innerHTML=i,l}function H(){let e=`<div class="fixed overflow-y-auto z-50 top-0 left-0 w-full h-full bg-gray-900 bg-opacity-80 pb-3">
              ...
              <div class="px-6 mb-10 pb-3">
                  <div class="pb-6 mb-6 border-b-2 border-black">
                      <div class="flex mb-6 pb-6 items-center justify-between border-b-2 border-black">
                          ...
                          <span class="text-sm font-black" id="shopemaa_cart-subtotal">0.00 `+c()+`</span>
                      </div>
                  ...
                  <div class="flex mb-6 items-center justify-between">
                      ...
                      <span class="text-lg font-black" id="shopemaa_cart-grand-total">0.00 `+c()+`</span>
                  </div>
      ...
      </div>`,a=document.createElement("section");a.classList.add("relative"),a.id="shopemaa_cartModal",a.innerHTML+=e,document.body.appendChild(a)}function U(){let e=s();e.items.length!==0&&(f(!1),e.shippingCharge=0,e.paymentFee=0,e.discount=0,e.paymentMethodId=null,e.shippingMethodId=null,y(e),z())}function z(){let e=s(),a="[";if(e.items.forEach(t=>{a+=`{ productId: "${t.id}" quantity: ${t.qty} }`}),a+="]",e.cartId===null){let t="mutation { newCart(params: { cartItems: "+a+" }) { id isShippingRequired } }";C(b(t),!0)}else{let t=`mutation { updateCart(id: "${e.cartId}" params: { cartItems: `+a+" }) { id isShippingRequired } }";C(b(t),!1)}}function C(e,a){e.then(t=>{if(!t.ok){d(`create or update cart response is: ${t.statusText}`),f(!0);return}t.json().then(o=>{if(o.data===null){d("create or update cart response body is NULL"),f(!0);return}if(a){let n=o.data.newCart.id,i=s();i.cartId=n,y(i)}v(!1),J()}).catch(o=>{f(!0),d(o)})}).catch(t=>{f(!0),d(t)})}function f(e){if(e){let a=document.getElementById("shopemaa_checkoutBtn");a.style.display="block";let t=document.getElementById("shopemaa_creatingCartBtn");t.style.display="none"}else{let a=document.getElementById("shopemaa_checkoutBtn");a.style.display="none";let t=document.getElementById("shopemaa_creatingCartBtn");t.style.display="block"}}function K(){let e=s(),a=0;e.items.forEach(l=>{a+=l.qty*m(l)}),document.getElementById("shopemaa_checkout-subtotal").innerText=(a/100).toFixed(2)+" "+c();let t=e.discount;document.getElementById("shopemaa_checkout-discount").innerText="-"+(t/100).toFixed(2)+" "+c();let o=e.shippingCharge;document.getElementById("shopemaa_checkout-shipping-charge").innerText=(o/100).toFixed(2)+" "+c();let n=e.paymentFee;document.getElementById("shopemaa_checkout-payment-fee").innerText=(n/100).toFixed(2)+" "+c();let i=((a+o+n-t)/100).toFixed(2);document.getElementById("shopemaa_checkout-grand-total").innerText=i+" "+c()}function J(){let e=`<div class="fixed overflow-y-auto z-50 top-0 left-0 w-full h-full bg-gray-900 bg-opacity-80">
    <form>
       <div class="relative ml-auto w-full max-w-lg bg-white">
          <div class="p-6 border-b-2 border-black">
             <h3 class="text-2xl font-bold">Your Cart</h3>
          ...
          <div class="px-6 mb-10 pt-2 pb-2">
             ...
             <div class="pb-6 mb-6 border-b-2 border-black">
                <div class="flex mb-6 pb-6 items-center justify-between border-b-2 border-black">
                   <span class="text-sm font-bold">Subtotal</span>
                   <span class="text-sm font-black" id="shopemaa_checkout-subtotal">0.00 ${c()}</span>
                </div>
                <div class="flex mb-3 items-center justify-between">
                   <span class="text-sm font-bold">Discount</span>
             ...
             <div class="flex mb-6 items-center justify-between">
                <span class="text-lg font-bold">Grand Total</span>
                <span class="text-lg font-black" id="shopemaa_checkout-grand-total">0.00 ${c()}</span>
             </div>
 ...
 </div>`,a=document.createElement("section");a.classList.add("relative"),a.id="shopemaa_checkoutModal",a.innerHTML+=e,document.body.appendChild(a),K(),W(),Y()}function W(){b("query { shippingMethods { id displayName deliveryCharge deliveryTimeInDays WeightUnit isFlat isActive } }").then(a=>{a.ok&&a.json().then(t=>{if(t.data!==null){let o=t.data.shippingMethods,n=document.getElementById("shopemaa_shippingMethod");o.forEach(i=>{let l=document.createElement("option");l.id=i.id,l.value=i.id,i.deliveryTimeInDays===1||i.deliveryTimeInDays===0?l.innerText=i.displayName+" (Delivering today)":l.innerText=i.displayName+` (Approx. delivery in ${i.deliveryTimeInDays})`,n.appendChild(l)})}}).catch(t=>{d(t)})}).catch(a=>{d(a)})}function Y(){b("query { paymentMethods { id displayName currencyName currencySymbol isDigitalPayment } }").then(a=>{a.ok&&a.json().then(t=>{if(t.data!==null){let o=t.data.paymentMethods,n=document.getElementById("shopemaa_paymentMethod");o.forEach(i=>{let l=document.createElement("option");l.id=i.id,l.value=i.id,l.innerText=i.displayName,n.appendChild(l)})}}).catch(t=>{d(t)})}).catch(a=>{d(a)})}function r(){return"gray"}function d(e){console.log(e)}function Q(){let e=864e5,a=function(){console.log("Performing cart cleanup");let t=s(),o=1e3*60*60*24,n=t.initAt;n==null&&h(),new Date().getMilliseconds()-n>=o&&h()};setInterval(a,e),a()}function Z(e){let a=`<div class="fixed overflow-y-auto z-50 top-0 left-0 w-full h-full bg-gray-900 bg-opacity-80 pb-3">
          <div class="relative ml-auto w-full h-full max-w-lg bg-white">
              ...
              <div class="px-6 mb-10 pb-3">
                  ...
                  <div class="pb-1 mb-1">
                      <b>Grand Total :</b> `+(e.grandTotal/100).toFixed(2)+" "+c()+`
                  </div>
              </div>
      ...
      </div>`,t=document.createElement("section");t.classList.add("relative"),t.id="shopemaa_orderModal",t.innerHTML+=a,document.body.appendChild(t)}function B(){let e=document.getElementById("shopemaa_orderSearchModal");e!==null&&e.remove()}function X(){let e=document.getElementById("shopemaa_order-hash").value,a=document.getElementById("shopemaa_customer-email").value,t=`query { orderByCustomerEmail(hash: "${e}", email: "${a}") { id hash shippingCharge paymentProcessingFee subtotal grandTotal discountedAmount status paymentStatus createdAt updatedAt billingAddress { id street streetTwo city state postcode email phone location { id name shortCode } } shippingAddress { id street streetTwo city state postcode email phone location { id name shortCode } } cart { isShippingRequired cartItems { product { id name slug description fullImages isDigitalProduct productUnit } quantity purchasePrice attributes { name selectedValue } variation { id name price sku stock } } } customer { email phone firstName lastName profilePicture } paymentMethod { id displayName currencyName currencySymbol isDigitalPayment } shippingMethod { id displayName deliveryCharge deliveryTimeInDays WeightUnit isFlat isActive } couponCode { code } payments { isPaid payableAmount gatewayName } } }`;b(t).then(o=>{o.ok&&o.json().then(n=>{n.data!==null&&(B(),Z(n.data.orderByCustomerEmail))}).catch(n=>{d(n)})}).catch(o=>{d(o)})}function ee(){let e=`<div class="fixed overflow-y-auto z-50 top-0 left-0 w-full h-full bg-gray-900 bg-opacity-80 pb-3">
          <div class="relative ml-auto w-full h-full max-w-lg bg-white">
              <div class="p-6 border-b-2 border-black">
      ...
      </div>`,a=document.createElement("section");a.classList.add("relative"),a.id="shopemaa_orderSearchModal",a.innerHTML+=e,document.body.appendChild(a)}(async function(){typeof window<"u"&&(window.shopemaa=window.shopemaa||{},window.shopemaa.hideOrderSearchModal=B,window.shopemaa.getOrderDetails=X,window.shopemaa.onGotoCheckout=U,window.shopemaa.toggleCartView=v)})(),p.initShopemaa=u,Object.defineProperty(p,Symbol.toStringTag,{value:"Module"})});
