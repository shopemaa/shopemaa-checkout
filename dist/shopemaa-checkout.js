...
function h() {
  localStorage.getItem("shopemaaCheckoutBucket") === null && u();
}
function u() {
  let e = {
    cartId: null,
...
function i() {
  ...
  return e === null && (h(), e = localStorage.getItem("shopemaaCheckoutBucket")), JSON.parse(e);
}
function B(e) {
  e.items = e.items.sort((t, a) => t.orderIndex - a.orderIndex), localStorage.setItem("shopemaaCheckoutBucket", JSON.stringify(e));
}
function C() {
  M(), q(), E();
}
...
function g() {
  let e = i(), t = document.getElementsByClassName("shopemaa-cart-items-count");
  if (t != null) {
    let a = t.length, l = 0;
    e.items.forEach((n) => {
      l += n.qty;
...
function T() {
  ...
  if (e != null) {
    let t = e.length;
    for (let a = 0; a < t; a++) {
      let l = e.item(a), n = i(), o = l.dataset.productId, s = n.items.find((k) => k.id === o);
      s != null ? e.item(a).innerText = s.qty : e.item(a).innerText = 0;
    }
  }
}
function q() {
...
function b(e, t) {
  i().items.forEach((l) => {
    let n = `shopemaa_cart_item_${l.id}`, o = document.getElementById(n);
    o != null && o.remove();
  }), f(e, t);
}
function f(e, t) {
  let a = i(), l = a.items.find((n) => n.id === e.id);
  l == null ? a.items.push({
    id: e.id,
    name: e.name,
    stock: e.stock,
    ...
    price: e.price,
    orderIndex: a.items.length + 1
  }) : l.qty + t <= e.stock && (l.qty += t, a.items = a.items.filter((n) => n.id !== e.id), l.qty > 0 && a.items.push(l)), B(a), x(!0), g();
}
function v(e) {
  let t = `query { product(productId: "${e}") { id name price stock fullImages productSpecificDiscount attributes { id name values isRequired } } }`;
  return y(t);
}
function y(e) {
  return fetch(O(), {
...
function D() {
  y("query { storeBySecret { name title description currency isOpen } }").then((t) => {
    t.ok && t.json().then((a) => {
      localStorage.setItem(
        "shopemaaStoreInfo",
        JSON.stringify(a.data.storeBySecret)
      );
    }).catch((a) => {
      r(a);
    });
...
function m(e) {
  let t = e.price;
  return e.productSpecificDiscount !== 0 && (t = t - e.productSpecificDiscount * t / 100), t;
}
function A() {
  let e = 0;
  i().items.forEach((t) => {
    e += t.qty * m(t);
  }), document.getElementById("shopemaa_cart-subtotal").innerText = (e / 100).toFixed(2) + " " + d();
}
function G() {
  let e = i(), t = 0;
  e.items.forEach((l) => {
  ...
  let a = t;
  document.getElementById("shopemaa_cart-grand-total").innerText = (a / 100).toFixed(2) + " " + d();
}
function x(e) {
  if (e) {
    let t = document.getElementById("shopemaa_cartModal");
    t == null && H();
    let a = document.getElementById("shopemaa_cartItemsView");
    i().items.forEach((l) => {
      a.appendChild(P(l));
      let n = document.getElementById(
        `shopemaa_qty_minus_${l.id}`
      );
      n.onclick = function() {
        b(l, -1);
      };
      let o = document.getElementById(`shopemaa_qty_plus_${l.id}`);
      o.onclick = function() {
        b(l, 1);
      };
    }), A(), G();
...
function P(e) {
  let t = e.fullImages[0], a = m(e), l = document.createElement("button");
  l.classList.add(
    "flex",
    "w-3.5",
    "h-3.5",
    ...
    "transition",
    "duration-100"
  ), l.innerHTML = '<div class="h-px mx-px w-full bg-white"></div>', l.id = `shopemaa_qty_minus_${e.id}`;
  ...
  let o = `<div class="flex mb-6 justify-between items-center">
                      ...
                      <div class="w-auto">
                          <span class="text-xl font-bold">${(a / 100).toFixed(2)} ${d()}</span>
                      </div>
                  </div>`, s = document.createElement("div");
  return s.id = `shopemaa_cart_item_${e.id}`, s.innerHTML = o, s;
}
function H() {
              ...
              <div class="px-6 mb-10 pb-3">
                  <div class="pb-6 mb-6 border-b-2 border-black">
                      <div class="flex mb-6 pb-6 items-center justify-between border-b-2 border-black">
                          ...
                          <span class="text-sm font-black" id="shopemaa_cart-subtotal">0.00 ` + d() + `</span>
                      </div>
                  ...
                  <div class="flex mb-6 items-center justify-between">
                      ...
                      <span class="text-lg font-black" id="shopemaa_cart-grand-total">0.00 ` + d() + `</span>
                  </div>
                  ...
                  <a id="shopemaa_checkoutBtn" onclick="event.preventDefault(); onGotoCheckout()"
                     class="group relative inline-block h-12 w-full bg-blueGray-900 rounded-md" href="#">
                      <div class="absolute top-0 left-0 transform -translate-y-1 -translate-x-1 w-full h-full group-hover:translate-y-0 group-hover:translate-x-0 transition duration-300">
                          <div class="flex h-full w-full items-center justify-center border-2 border-black rounded-md transition duration-300 bg-` + c() + `-600">
                  ...
                  <a onclick="event.preventDefault(); toggleCartView(false);"
                     class="group relative inline-block h-12 w-full bg-blueGray-900 rounded-md mt-2" href="#">
                      <div class="absolute top-0 left-0 transform -translate-y-1 -translate-x-1 w-full h-full group-hover:translate-y-0 group-hover:translate-x-0 transition duration-300">
                          <div class="flex h-full w-full items-center justify-center border-2 border-black rounded-md transition duration-300 bg-` + c() + `-400">
  ...
  t.classList.add("relative"), t.id = "shopemaa_cartModal", t.innerHTML += e, document.body.appendChild(t);
}
function c() {
  return "gray";
}
...
function N() {
  let e = 864e5, t = function() {
    console.log("Performing cart cleanup");
    let a = i(), l = 1e3 * 60 * 60 * 24, n = a.initAt;
    n == null && u(), (/* @__PURE__ */ new Date()).getMilliseconds() - n >= l && u();
  };
  setInterval(t, e), t();
}
function V() {
...
function F() {
          ...
          <div class="relative ml-auto w-full h-full max-w-lg bg-white">
              ...
              <div class="px-6 mb-10 pb-3">
                  ...
                  <a onclick="event.preventDefault(); shopemaa.hideOrderSearchModal()" class="pt-2 mt-2 pr-5 pl-5 group relative inline-block h-12 w-full bg-blueGray-900 rounded-md" href="#">
                    <div class="absolute top-0 left-0 transform -translate-y-1 -translate-x-1 w-full h-full group-hover:translate-y-0 group-hover:translate-x-0 transition duration-300">
                       <div class="flex h-full w-full items-center justify-center border-2 border-black rounded-md transition duration-300 bg-` + c() + `-100">
  ...
  t.classList.add("relative"), t.id = "shopemaa_orderSearchModal", t.innerHTML += e, document.body.appendChild(t);
}
(async function() {
  typeof window < "u" && (window.shopemaa = window.shopemaa || {}, window.shopemaa.hideOrderSearchModal = V);
})();
export {
  V as hideOrderSearchModal,
  J as initShopemaa
};
